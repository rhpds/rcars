...
db_pull() {
    ...
    podman exec -i "${PG_CONTAINER}" psql -U rcars -c "SELECT pg_terminate_backend(pid) FROM pg_stat_activity WHERE datname = 'rcars' AND pid <> pg_backend_pid();" postgres >/dev/null 2>&1
