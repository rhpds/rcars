# Graph Report - rcars  (2026-08-02)

## Corpus Check
- 158 files · ~283,435 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 1687 nodes · 3221 edges · 105 communities (86 shown, 19 thin omitted)
- Extraction: 93% EXTRACTED · 7% INFERRED · 0% AMBIGUOUS · INFERRED: 222 edges (avg confidence: 0.61)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `ffda34b7`
...
### Community 2 - "cli.py"
Cohesion: 0.13
Nodes (30): argument, command, flag(), get_db(), infra_stats(), note(), override_url(), RCARS CLI — RHDP Content Advisory & Recommendation System. (+22 more)

### Community 3 - "pipeline.py"
Cohesion: 0.10
Nodes (31): PostgreSQL + pgvector database layer for RCARS v2., Candidate, QueryState, Data models for the recommendation pipeline., A content entity moving through the recommendation pipeline., Convert similarity score (0.0-1.0) to percentage., State of a recommendation query at a pipeline phase boundary., _apply_duration_penalty() (+23 more)
...
Cohesion: 0.10
Nodes (29): CompletedProcess, build_analysis_prompt(), clone_showroom(), _is_github_throttle(), ls_remote_sha(), Showroom content analyzer. Clones Showroom repos, reads AsciiDoc content, sends…, Truncate content to max characters., Run a git command with retry and exponential backoff for GitHub throttling. (+21 more)
...
Cohesion: 0.05
Nodes (55): asyncio, BoundLogger, Exception, Redis, RedisSettings, stream_query(), create_sse_response(), JobProgressRelay (+47 more)
...
### Community 11 - "compute_content_similarity"
Cohesion: 0.12
Nodes (35): _build_similar_item(), compute_content_similarity(), get_overlap_items(), get_similar_items(), get_similarity_stats(), Any, ConnectionPool, Content similarity computation and queries. Extracted from database.py to start… (+27 more)
...
Cohesion: 0.12
Nodes (11): BaseSettings, parametrize, _parse_csv(), Settings, migrate(), One-time migration: export token_usage from old schema, create new schema,…, TestUnauthenticatedAccess, test_admin_check() (+3 more)
...
Cohesion: 0.05
Nodes (41): Advisor Chat — Multi-Intent Design, API Surface, Approach Decision, Architecture, Backend modules, Configuration, Context builder, Deployment (+33 more)

### Community 19 - "schemas.py"
Cohesion: 0.11
Nodes (31): health(), get, Request, readiness(), AuthMeResponse, CatalogItemWorkload, CatalogListResponse, ErrorDetail (+23 more)
...
Cohesion: 0.20
Nodes (20): invalidate_api_key_cache(), auth_me(), create_api_key(), exchange_token(), _generate_api_key(), list_api_keys(), delete, get (+12 more)
...
Cohesion: 0.22
Nodes (18): _advisor_limit(), get_query_result(), get_session(), list_sessions(), BaseModel, get, limit, post (+10 more)
...
### Community 33 - "admin.py"
Cohesion: 0.26
Nodes (19): compute_similarity(), get_job(), list_jobs(), llm_provider_status(), overlap_report(), get, post, Request (+11 more)
...
Cohesion: 0.07
Nodes (24): BaseHTTPMiddleware, datetime, FastAPI, create_app(), lifespan(), Request, RequestLoggingMiddleware, client() (+16 more)
...
Cohesion: 0.10
Nodes (22): Catalog Backfill (zero-value items for full coverage), content_entities/babylon_items Two-Table Design, content_id Source-Prefixed Identity Scheme, Cost Methodology: All-Environment Cost Amortized to Prod, Jira Retirement Ticket Integration, Percentile-Based Retirement Scoring, Why provisions_summary Instead of Raw provisions Table, Retirement Workflow Lifecycle (approve→notify→start→retired) (+14 more)
...
Cohesion: 0.18
Nodes (13): Acronym Expansion (AAP, RHOAI, OCP, etc.), CI Name Resolution in Vector Search, Duration-Aware Reranking (soft/hard constraint), Scan Error Classification (jinja_url, timeout, etc.), Event URL Mode (fetch + extract + search), No-Match Behavior (fail honestly vs widen cutoff), Published/Base CI Promotion, Three-Phase Progressive Recommendation Pipeline (+5 more)
...
### Community 49 - "analyze_showroom"
Cohesion: 0.15
Nodes (20): analyze_showroom(), build_module_embedding_text(), check_showroom_stale(), filter_boilerplate_files(), get_repo_head(), hash_showroom_content(), parse_analysis_response(), _parse_nav_includes() (+12 more)
...
Cohesion: 0.17
Nodes (15): _call_anthropic(), _call_litemaas(), call_llm(), LLMResult, Unified LLM call with automatic provider routing. LiteMaaS preferred if…, _build_deterministic_assessment(), _call_rationale_single(), _call_synthesis() (+7 more)
...
### Community 55 - "workload_mapping table (curated role → product name)"
Cohesion: 0.25
...
### Community 60 - "reporting_db_show"
Cohesion: 0.24
Nodes (10): pass_context, _print(), Sync reporting metrics from RHDP MCP server., Show reporting sync status and score distribution., Show performance metrics for a content entity (accepts ci_name, content_id, or…, Refresh catalog from Babylon CRDs (all namespaces)., refresh(), reporting_db_show() (+2 more)

### Community 61 - "rcars-login.py"
Cohesion: 0.54
Nodes (7): cmd_logout(), _load_credentials(), main(), cmd_login(), cmd_status(), cmd_token(), _save_credentials()

### Community 62 - "RCARS Generalized Content Model — Implementation Plan"
Cohesion: 0.22
...
### Community 66 - "option"
Cohesion: 0.22
Nodes (9): option, cli(), compute_similarity_cmd(), init_db(), Compute pairwise content similarity (overlap + related pairs)., RCARS — RHDP Content Advisory & Recommendation System., Initialize or reset the database schema., Start the RCARS API server. (+1 more)

### Community 67 - "rcars-login.sh"
Cohesion: 0.48
Nodes (5): json_get(), rcars-login.sh script, cmd_login(), cmd_status(), cmd_token()

### Community 68 - "fetch_litemaas_models"
Cohesion: 0.22
Nodes (8): Analyze Showroom content via Sonnet API., Scan agDv2 workload repos, analyze roles via LLM, update mappings., scan(), workload_scan(), fetch_litemaas_models(), Query LiteMaaS /v1/models endpoint once and cache the result., Scan all (or filtered) agDv2 collection repos., scan_all_collections()

### Community 69 - "test_services.py"
Cohesion: 0.22
...
### Community 72 - "ApiKeysPanel.tsx"
Cohesion: 0.60
Nodes (4): ApiKeyRow, ApiKeysPanel(), expiryLabel(), timeAgo()

### Community 73 - "rate_limit.py"
Cohesion: 0.50
...
### Community 78 - "group"
Cohesion: 0.29
Nodes (7): group, infra_group(), Infrastructure metadata commands., Workload mapping and scanning commands., Reporting database metrics commands., reporting_db_group(), workload_group()

### Community 98 - "RCARS Rearchitecture & API Design"
Cohesion: 0.33
...
### Community 99 - "build_sandbox_summary"
Cohesion: 0.40
Nodes (4): build_sandbox_summary(), Sandbox summary generation from infrastructure metadata and workload…, Assemble a sandbox summary from infrastructure metadata. workload_products:…, run_sandbox_summary()

### Community 100 - "db"
Cohesion: 0.40
Nodes (5): db(), fixture, Create a fresh test database with schema and seed data., Insert content_entities, babylon_items, and embeddings for testing., _seed_test_data()

### Community 102 - "format_triage_candidates"
Cohesion: 0.67
Nodes (3): format_triage_candidates(), Candidate, Format candidates compactly for the triage prompt.

## Knowledge Gaps
- **232 isolated node(s):** `RCARS_DATABASE_URL`, `RCARS_REDIS_URL`, `RCARS_DEV_USER`, `RCARS_ADMIN_EMAILS_STR`, `RCARS_CURATOR_EMAILS_STR` (+227 more)
...
- **Why does `Content Overlap Detection (doc)` connect `Content Overlap Detection (doc)` to `System Design (doc)`, `compute_content_similarity`, `Recommendation Engine (doc)`?**
  _High betweenness centrality (0.181) - this node is a cross-community bridge._
- **Why does `Database` connect `Database` to `cli.py`, `pipeline.py`, `analyzer.py`, `settings.py`, `compute_content_similarity`, `derive_status`, `Settings`, `test_db.py`, `Any`, `_generate_key`, `config.py`, `extract_base_name`, `reporting_db_show`, `option`, `fetch_litemaas_models`, `.complete_job`, `.retire_removed_items`, `.__init__`, `.prune_expired_api_keys`, `.prune_old_jobs`, `.revoke_user_cli_keys`, `db`?**
  _High betweenness centrality (0.148) - this node is a cross-community bridge._
- **Why does `System Design (doc)` connect `System Design (doc)` to `Content Overlap Detection (doc)`, `Data Design (doc)`, `Recommendation Engine (doc)`?**
