...
function ScheduledMaintenance() {
  ...
  const handleRunBabylon = async () => {
    ...
    addLog('Starting Babylon pipeline (catalog sync → stale check → re-analyze → workload scan → reporting sync → overlap detection)...')
    try {
    ...
    <div className="admin-section">
      ...
      <div style={{ fontSize: '12px', color: 'var(--text-muted)', marginBottom: '10px', lineHeight: '1.7' }}>
        ...
        <ol style={{ margin: '0 0 8px 20px', listStyleType: 'decimal' }}>
          ...
          <li><strong>Reporting Sync</strong> — pulls performance metrics and field source provisions from the reporting database</li>
          <li><strong>Overlap Detection</strong> — generates and assesses content overlap candidates</li>
        </ol>
