...
class Database:
    ...
    def get_infrastructure_with_item_counts(
        ...
        base_sql = f"""
            ...
            FROM infrastructure i
            LEFT JOIN (
                SELECT biw.workload_role AS role_name, COUNT(DISTINCT COALESCE(bi.base_ci_name, bi.ci_name)) AS cnt
                FROM babylon_item_workloads biw
            ...
            LEFT JOIN (
                SELECT bi.agd_config AS role_name, COUNT(DISTINCT COALESCE(bi.base_ci_name, bi.ci_name)) AS cnt
                FROM babylon_items bi
                JOIN content_entities ce ON ce.content_id = bi.content_id AND ce.retired_at IS NULL
