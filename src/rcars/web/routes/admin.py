...
def _log_block_html(lines: list[str], div_id: str = "", collapsed: bool = False) -> str:
    ...
    return (
        ...
        f'<div id="{div_id}" '
        f'onscroll="var p=window._rcarsLogPos=window._rcarsLogPos||{{}};'
        f'p[\\"{div_id}\\"]={{top:this.scrollTop,atBottom:this.scrollHeight-this.scrollTop-this.clientHeight<50}};" '
        f'style="margin-top:6px;background:var(--bg-card);'
