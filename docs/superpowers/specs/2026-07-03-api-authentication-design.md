...
#### Proxy verification secret delivery

A Kubernetes Secret provides the shared proxy verification secret to both the frontend nginx and the API:
...
The current line 51 of `nginx.conf`:
```nginx
proxy_set_header X-Proxy-Secret $http_x_proxy_secret;   # pass-through (current)
```
Changes to:
```nginx
proxy_set_header X-Proxy-Secret "${PROXY_SECRET}";       # injected from mounted secret
```

**API pod** — the same secret is mounted or injected as `RCARS_PROXY_VERIFICATION_SECRET` env var (same pattern as existing secrets in `manifests-infra.yaml.j2`).

Both deployments reference the same K8s Secret, so the value is consistent and rotated in one place. The secret value is generated by Ansible from the vars file (gitignored) and stored in the cluster — it never appears in the nginx.conf source or any committed file.
